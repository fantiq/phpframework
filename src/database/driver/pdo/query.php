...
class query extends DB{
	...
	public function update($tablename='',$data=[], $cond=null){
		...
		$this->params = array_merge($tmpValueArr,$this->params);
	...
	public function exec(){
		...
		file_put_contents('./log/logs.txt', $this->sql."\r\n", FILE_APPEND);
		file_put_contents('./log/logs.txt', print_r($this->params,true)."\r\n", FILE_APPEND);
		$this->stmt = $this->dbh->prepare($this->sql);
		if($this->stmt->execute($this->params)===false){
			exit($this->stmt->errorInfo()); // 这里要将错误内容增加到Exception中，并抛出
		}
		// 清空数据
		$this->params = [];
		$this->queryString = ['select'=>['field'=>'*','from'=>'','join'=>'','order'=>'','group'=>'','limit'=>''],'insert'=>'','update'=>'','delete'=>'','where'=>'',];
		return true;
	}
}